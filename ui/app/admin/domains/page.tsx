...
import { Trash2, Plus, X, Globe, ToggleLeft, ToggleRight, GripVertical } from "lucide-react";
...
export default function AdminDomainsPage() {
  ...
  const dragProps = (idx: number) => ({
    ...
    onDragOver: (e: React.DragEvent) => { e.preventDefault(); dragOverIdx.current = idx; },
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent text-white text-sm hover:bg-accent-hover transition-colors">
      ...
      {showForm && (
        <div className="border border-border rounded-lg p-4 space-y-4 bg-bg-subtle">
          ...
          <div>
            ...
            <input type="text" value={formDomain} onChange={(e) => setFormDomain(e.target.value)} placeholder="example.com" className={inputClass} />
          </div>
          <button onClick={handleAdd} disabled={saving || !formDomain.trim()} className="px-4 py-2 rounded-lg bg-accent text-white text-sm hover:bg-accent-hover transition-colors disabled:opacity-50">
            {saving ? "Adding..." : "Add"}
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="hidden sm:block border border-border rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {domains.map((d, idx) => (
                <tr key={d.id} {...dragProps(idx)} className="border-b border-border hover:bg-bg-hover transition-colors cursor-grab active:cursor-grabbing">
                  <td className="px-2 py-2.5 text-center">
                    <GripVertical className="w-4 h-4 text-fg-muted inline-block" />
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-fg-muted font-mono text-xs">{d.cf_zone_id || "—"}</td>
        ...
        <div className="sm:hidden space-y-2">
          {domains.map((d, idx) => (
            <div key={d.id} {...dragProps(idx)} className="border border-border rounded-lg p-3 space-y-2 cursor-grab active:cursor-grabbing touch-none">
              <div className="flex items-center justify-between">
        ...
        <p className="text-xs text-fg-muted text-center">Drag to reorder. The first enabled domain is the default.</p>
        </>
      )}
    </div>
  );
